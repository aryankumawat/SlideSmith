...
export async function POST(request: NextRequest) {
  try {
    ...
    // In production, this would use Playwright to render the slides
    const pdfBuffer = await generatePDF(deck);
    
    return new NextResponse(pdfBuffer as any, {
      headers: {
        'Content-Type': 'application/pdf',
        'Content-Disposition': `attachment; filename="${deck.meta.title.replace(/[^a-zA-Z0-9]/g, '_')}.pdf"`,
      },
    });
  } catch (error) {
...
async function generatePDF(deck: Deck): Promise<Buffer> {
  ...
  // 1. Use Playwright to render the slides
  // 2. Convert to PDF
